...
type Thread struct {
	...
	id		uint64
//	tags		[]Tag
	posts		[]*Post	/* First post is body of thread. */
	next, prev	*Thread
}
	...
	if first_post == nil { /* This is the first post. */
		...
		if cp.next != nil {
			...
			go func () {
				...
				err = ioutil.WriteFile(strconv.FormatUint(np.id, 32), b, 0644)
				if err != nil {
					fmt.Println(err)
					return
				}
...
func getThread(id uint64) *Thread {
	...
	if t, ok = qlookup[id]; ok == false {
		...
		b, err := ioutil.ReadFile(strconv.FormatUint(id, 32))
		if err != nil {
